...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{res/03-reno-cubic/analysis/temp-phase-fitting.pdf}
    \caption{Regressionsbasierte Analyse: Congestion Avoidance Fits}\label{fig:phase_fitting}
\end{figure}

Für TCP Reno bestätigt der lineare Fit den AIMD Mechanismus mit einem mittleren $\bar{R}^2 = 0{,}959$ über 22 Regime.
Ein exponentieller Fit für die Slow-Start Phase ist für Reno nicht darstellbar, da der erste Paketverlust bereits zu Beginn auftritt ($<$ 3 RTT).

Für TCP CUBIC ergibt der kubische Fit $\bar{R}^2 = 0{,}956$ über 16 Regime.
Das Wachstum folgt der kubischen Funktion $W(t) = C \cdot (t - K)^3 + W_{\max}$ (vgl. CUBIC-RFC~\cite{tcp_cubic}).
Die Slow-Start Phase umfasst lediglich vier RTTs ($R^2 = 0{,}845$, Verdopplungszeit $T_2 \approx 240$\,ms) und wird durch den ersten Verlust abgebrochen.
...
Dies ist auf das niedrige Bandbreiten-Verzögerungs-Produkt (BDP $\approx 1\,\text{Mbit/s} \times 80\,\text{ms} \approx 10$\,kB) zurückzuführen: beide Algorithmen füllen die verfügbare Kapazität, bevor CUBICs aggressiveres Wachstum zum Tragen kommen kann.
