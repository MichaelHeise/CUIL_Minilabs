...
Dabei werden durch \texttt{tcp.\allowbreak{}analysis.\allowbreak{}retransmission} alle Pakete gefiltert, die von TCP als Retransmission erkannt werden.
...
Dieses ruft nacheinander \texttt{tcptrace-\allowbreak{}util.sh} (in einem \texttt{debian:\allowbreak{}bookworm-\allowbreak{}slim} Docker Container), \texttt{tshark-\allowbreak{}util.sh} und \texttt{analyze.py} (via \texttt{uv run}) auf.
...
Der mittlere Durchsatz wird von iperf3 als \texttt{bits\_\allowbreak{}per\_\allowbreak{}second} unter \texttt{end.\allowbreak{}sum\_\allowbreak{}sent} in \texttt{iperf3\_\allowbreak{}\{reno,\allowbreak{}cubic\}.\allowbreak{}json} dargestellt.
...
TCP CUBIC zeigt ein ähnliches Sägezahnmuster wie Reno, jedoch mit geringerer Reduktion nach Verlusten ($\beta = 0{,}7$ statt $0{,}5$).
Das theoretisch erwartete kubische Wachstumsverhalten mit Abflachung nahe $W_{\max}$ ist hier nicht erkennbar.
...
Tabelle~\ref{tab:reno_cubic_summary} fasst die wichtigsten Kennwerte beider Algorithmen zusammen (Entnommen aus \texttt{iperf3\_\allowbreak{}\{reno,\allowbreak{}cubic\}.\allowbreak{}json}, \texttt{\{reno,\allowbreak{}cubic\}-\allowbreak{}summary.\allowbreak{}txt} und \texttt{parse\_\allowbreak{}packet\_\allowbreak{}losses()} in \texttt{parsers.py}).
...
Ziel ist es, das theoretisch erwartete Wachstumsverhalten quantitativ anhand der Messdaten zu überprüfen.
Abbildung~\ref{fig:phase_fitting} zeigt die cwnd Verläufe beider Algorithmen mit Regressionskurven.
Dabei wird für jedes Congestion Avoidance Regim (Wachstumsphase zwischen zwei Verlustereignissen) ein Polynom des passenden Grades gefittet: linear (Grad~1) für Reno, kubisch (Grad~3) für CUBIC.
...
Dies kann auf das niedrige BDP (BDP $\approx 1\,\text{Mbit/s} \times 80\,\text{ms} \approx 10$\,kB) zurückführbar sein. Beide Algorithmen füllen die Kapazität, bevor CUBICs Wachstum dominieren kann.
...
Widersprüchlich ist diese Metrik auch unter der Annahme, dass Outstanding~$\approx$~cwnd sein sollten, \texttt{max owin} und \texttt{max snd\_cwnd} ähnlich ausfallen, doch \texttt{max owin} liegt bei 70.953\,Bytes (Reno) bzw.\ 69.505\,Bytes (CUBIC) (vgl.\ Abschnitt~\ref{sec:reno-cubic-ergebnisse}).

\subsubsection{Praktischer Einsatz}

Die geringe Diskrepanz der Algorithmen kann als eine Folge des niedrigen BDPs beziehungsweise der begrenzten zeitlichen Beobachtung interpretiert werden.
